...
{score_instructions}
PS.
- 任何 typo，請整題只扣他最多 1 分，就算他多個地方都 typo 整題也只合計扣 1 分。你只應該抓 output 的 typo，其他 input 的提示字，甚至是註解、程式碼變數命名等地 typo 都不管
- 所有輸入的 edge case 都會有測資涵蓋，如果不在測資內出現也不在題目要求，則不需要考慮，尤其是輸入非預期情況，這種事情不會發生
- 不在乎用什麼方式輸入，不在乎輸入的安全性與否(例如 gets)，除非題目有要求，或者他使用的輸入方式不對，導致無法正確處理題目要求邏輯(例如: 某些題目要保留空格等要求)，否則你不應該扣分
