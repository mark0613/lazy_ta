...
3. 輸出格式 (6 分):
  - 必須使用 first, second, third 來表示月份否則該項目 0 分，例如他直接寫 1st, 2nd, 3th。
  - 如果 first, second, third 這些關鍵字拼錯字，或者前兩個正確最後一個誤植(例如 first, second, first)，請算在 typo
  - 其他如空格對齊、$ 符號、多餘空格等格式上的小問題不扣分

PS. 以 6 分為一個檔次，意思是數學邏輯部分錯誤，當你要酌扣分數，就是 6、12、18 分這樣，不能只扣 1、2 分
"""

EXTRA_INSTRUCTIONS = """
- 輸出格式:
  - 前三個必須使用 first, second, third 來表示月份否則該項目 0 分，例如他直接寫 1st, 2nd, 3th。
  - 如果 first, second, third 這些關鍵字拼錯字，或者前兩個正確最後一個誤植(例如 first, second, first)，請算在 typo
  - 其他如空格對齊、$ 符號、多餘空格等格式上的小問題不扣分
