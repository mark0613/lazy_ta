...
SCORE_INSTRUCTIONS = """
總共 30 分，具體評分標準如下：
1. 輸入格式 (5 分): 能正確讀取並處理使用者輸入，主要抓讀取方式錯誤或無法處理測試案例的情況
2. 數學邏輯 (20 分): 計算每月餘額的邏輯正確
3. 輸出格式 (5 分): 能正確顯示每月餘額，並保留兩位小數(這是重點，沒保留兩位小數，視為輸出格式錯誤，給他 0 分)

PS. 以 5 分為一個檔次，意思是數學邏輯部分錯誤，你要酌扣分數，就是 5、10、15、20 分這樣，不能扣 1、2 分
"""

EXTRA_INSTRUCTIONS = """
- 如同 Output Example，請確保每月餘額前有 "$" 符號，且數字對齊，若沒有則視為格式錯誤，和 typo 合計只扣最多 1 分
- 輸出格式必須和 Output Example 一致，只能用 first, second, third 來表示月份，若使用其他方式表示，則輸入格式的項目給他 0 分
"""
...
- 前三筆只能是 first, second, third，之後的筆數請用數字表示 (e.g., 4th month, 5th month, etc.)，如果沒有照這個規則，視為格式錯誤，直接 0 分
"""
)
